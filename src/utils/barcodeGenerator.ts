 ...
 * Pure TypeScript, zero external dependencies, sharp vector output for thermal & laser printers.
 */
...
export function generateCode128Svg(text: string, height: number = 50, barWidth: number = 2): string {
  ...
  const svgWidth = (totalModules + 20) * barWidth;
  let currentX = 10 * barWidth;
  let rects = '';
  ...
  return `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${svgWidth} ${height + 20}" width="100%" height="${height + 20}">
      ${rects}
      <text x="${svgWidth / 2}" y="${height + 15}" font-family="monospace" font-size="12" font-weight="bold" text-anchor="middle" fill="#000000">${cleanText}</text>
    </svg>
  `;
}
